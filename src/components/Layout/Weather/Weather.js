...
const weather = props => {
    ...
    let uppderCaseDescription = description.charAt(0).toUpperCase() + description.slice(1);
    let windSpeed = Math.floor(props.currentWeather.wind.speed * 3.6);
    let humidity = props.currentWeather.main.humidity;

    const prefix = "wi wi-";
    const icon = prefix + weatherIcons.default[props.currentWeather.weather[0].id].icon;
    const recommendation = recommendations.default[props.currentWeather.weather[0].id].recommendation;
    return (
      <div className={classes.Weather}>
        <div className={classes.WeatherHeader}>
          <span>{location}, {country}</span>
          <span>{day}, {formattedTime}, {uppderCaseDescription}</span>
        </div>
        ...
        <div className={classes.row}>
          <div className={classes.col}>
            <h2>{temp}&deg;C</h2>
            <h4 className="">
              <span className="wi wi-strong-wind jss286 jss275">
                {" "}
                {windSpeed}km/h Winds
              </span>
              <span className="wi wi-humidity jss286 jss275">
                {" "}
                {humidity}% Humidity
              </span>
            </h4>
